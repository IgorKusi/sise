...
$Progcmd = 'java -jar D:\SISE\repo\JAVA\solutions\artifacts\SISE_jar\SISE.jar'
...
function RunProg([string]$strategy, [string]$param) {
    Get-ChildItem -File | Where-Object { $_.Name -match $InitFilenameRegex } | ForEach-Object {
        ...
        $SolFilename = 'D:\SISE\repo\JAVA\out\{0}_sol.txt' -f $FilenameRoot
        $StatsFilename = 'D:\SISE\repo\JAVA\stats\{0}_stats.txt' -f $FilenameRoot
